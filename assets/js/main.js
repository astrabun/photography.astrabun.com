...
(function ($) {
    ...
    $(function () {
        ...
        function isArrayofBytes(arr) {
            return Array.isArray(arr) && arr.every(function(item) {
                return typeof item === 'number' && item >= 0 && item <= 255;
            });
        }
        ...
        function getExifDataMarkup(img) {
            ...
            for (var current in exif) {
                ...
                if(isArrayofBytes(exif_data)){
                    // var filteredByteArray = exif_data.filter(function(byte) {
                    //     return byte !== 0;
                    // });
                    var lastZeroIndex = exif_data.lastIndexOf(0);
                    var relevantByteArray = exif_data.slice(lastZeroIndex + 1);
                    exif_data = String.fromCharCode.apply(null, relevantByteArray);
                }
